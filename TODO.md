07-29 thoughts

- [X] 把chat整合到主程序中
- [X] 让模型掉到窗口上的时窗口向下弹一下

为桌宠添加窗口碰撞交互效果：当桌宠掉落窗口边缘并被窗口接住时时，窗口会向下弹几下。我们假设窗口的质量和窗口的大小成正比，窗口向下弹几下，但是必须恢复到原处。可以使用windows_utils.py中的transformWindow实现。物理计算可以使用physics.py中的物理引擎实现

- [X] ~~模型可以更流畅地做出行为，比如自己有意识看向鼠标~~
- [X] ~~添加好感度~~
- [X] 添加历史记录
- [X] 尝试添加TTS，最好是流式
- [ ] ~~增加联网搜索功能。最好能显示动画，浏览器窗口弹出，模型跳到浏览器里，然后操作浏览器，最后能跳到最上面去按关闭浏览器按钮~~
- [X] jump_to，climb_to这种行为添加动画，jump_to模型要面向位置跳跃而不是面对屏幕跳跃
- [X] 修复无法在全屏窗口显示的bug

07-30 thoughts

- [X] tray界面美化

现在窗口打开后大小是个正方形，有点诡异，可以设置成800x600吗。然后设置界面一些设置都是被QGroupBox什么圈起来的，比较丑陋，我不希望看到圈起来的设置。然后QMessageBox.warning和QMessageBox.information请使用qfluentwidgets中的dialog，可以在./examples/dialog_flyout/dialog中查看用法。

- [X] 解决高DPI问题
- [X] 给pickup动画加回来
- [X] 添加对话界面（思考一下在哪里添加，设置窗口？右键桌宠显示对话框？）
- [X] ~~给桌宠添加阴影~~

08-01 thoughts

* [X] 如果输出的action list中含chat，那么最好是chat在说话之后的同时执行其他task，因为chat会花很长时间，如果是比如先jump一下，然后思考个一分多钟再说话就很奇怪
* [X] 加“摸头”功能

08-02 thoughts

* [X] 不应该是停止拖拽桌宠就立刻播放idle动画，应该是一直播放formation pickup直到落地/碰撞

08-03 thoughts

* [X] 不应该把sentence transformers那个model和pet写在一起，应该新开一个sentence_api.py的fastapi代码单独运行
* [ ] 给环境最好加上一键运行的脚本
* [X] 若用户长时间不回应，那么延长brain loop中的SLEEP time时间，同时添加AI检测用户回复率的脚本，如果长时间不回复那么自己主动延长SLEEP时间。用户也可以设置桌宠是偏向于说话还是观察
* [ ] 使用uv管理项目
